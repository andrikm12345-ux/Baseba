...
- ERA разница > 1.5 между стартерами при прочих равных
- Один питчер явно элита (ERA < 3.00, WHIP < 1.10), другой слабый (ERA > 5.00)
- Команда выиграла 8–10 из последних 10 против аутсайдера, кэф > 2.0
- Стадион (Coors Field) + оба питчера слабые → OVER

Что НЕ даёт перевеса:
- «Команда в хорошей форме» — рынок это уже знает
- ERA разница < 0.75 — статистический шум
- Один питчер чуть лучше — рынок это отразил в кэфах
- Просто «кажется» что выиграет

## РЕШЕНИЕ

Сначала спроси себя: есть ли КОНКРЕТНЫЙ факт выше, который рынок не дооценил?
- Если ДА → выбери рынок с наибольшим расхождением (твоя % − рынок%), confidence ≥ 0.62, расхождение ≥ 7%
- Если НЕТ → верни confidence: 0.50 (пропуск)

БОЛЬШИНСТВО игр должны получить пропуск. Это правильно.
...
confidence: 0.50 = пропуск, 0.62–0.90 = сигнал\
"""
