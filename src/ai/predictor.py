...
2) Если данные питчеров отсутствуют (н/д) — критерии A и B недоступны, но C и D всё ещё применяются. Никогда не ставь только потому что "нет данных" — это пропуск.
3) ТОЛЬКО следующие Fact-критерии дают шанс на сигнал:
   - Критерий A (разница стартеров): |ERA_home - ERA_away| > 1.5
   - Критерий B (элита vs слабый): один питчер ERA < 3.00 и WHIP < 1.15, другой ERA > 5.00
   - Критерий C (перекос цены): команда выиграла 8–10 из 10 последних, её кэф > 2.10 при этом (рынок не отреагировал)
   - Критерий D (стадион): Coors Field и оба ERA > 4.50 → OVER
4) Что НЕ даёт перевеса: ERA разница < 1.5; "в хорошей форме" без цифр; один питчер чуть лучше; интуиция.
5) Оценка перевеса:
   - Рассчитай P_exp по активным критериям. Без ни одного критерия P_exp = 0.50, сигнала нет.
   - Возьми P_mkt из no-vig для выбранного рынка/стороны.
   - Сигнал = P_exp >= 0.63 И |P_exp - P_mkt| >= 0.06.
   - Иначе — пропуск (confidence 0.50).
6) Приоритет рынков: ML > RUN_LINE > TOTAL. При равной уверенности — максимальный |P_exp - P_mkt|.
7) Confidence:
